...
class EventStore:
    ...
    def search_by_date(self, value: str | date | datetime) -> List[EventRecord]:
        ...
        if not target:
            return []
        return [record for event_id in self._order if (record := self._events[event_id]).date == target]
    ...
    def search_by_time(self, value: str | time | datetime) -> List[EventRecord]:
        ...
        if not target:
            return []
        return [record for event_id in self._order if (record := self._events[event_id]).time == target]
