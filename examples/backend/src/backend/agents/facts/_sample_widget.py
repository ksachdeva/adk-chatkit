...
WEATHER_ICON_SOURCES: dict[str, str] = {name: _encode_svg(svg) for name, svg in WEATHER_ICON_SVGS.items()}
...
def _sunrise_detail_svg(rising: bool) -> str:
    ...
    rays_top = '<line x1="32" y1="10" x2="32" y2="6"/>' if rising else '<line x1="32" y1="44" x2="32" y2="48"/>'
    return (
        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" fill="none">'
...
DETAIL_ICON_SOURCES: dict[str, str] = {name: _encode_svg(svg) for name, svg in DETAIL_ICON_SVGS.items()}
...
def weather_widget_copy_text(data: WeatherWidgetData) -> str:
    ...
    if time_text:
        segments.append(f"As of {time_text}, {location_text} is {temperature_text} with {condition_text}.")
    else:
        segments.append(f"Current weather in {location_text} is {temperature_text} with {condition_text}.")
    ...
    if data.hourly:
        ...
        for forecast in list(data.hourly)[:4]:
            ...
            temp_label = _format_temperature(forecast.temperature, forecast.temperature_unit or data.temperature_unit)
...
def _build_details_section(data: WeatherWidgetData) -> WidgetComponent | None:
    ...
    chips = _compact(
        [
            ...
            _detail_chip("Precipitation", precipitation_text, "umbrella") if precipitation_text else None,
...
def _build_hourly_section(data: WeatherWidgetData) -> WidgetComponent | None:
    ...
    cards = [_hourly_chip(forecast, data.temperature_unit, data.timezone_abbreviation) for forecast in data.hourly]
...
def _hourly_chip(forecast: HourlyForecast, default_unit: str, timezone_abbreviation: str) -> WidgetComponent:
    time_label = _format_hour_label(forecast.time, timezone_abbreviation)
    temperature_label = _format_temperature(forecast.temperature, forecast.temperature_unit or default_unit)
...
def _format_updated_label(moment: datetime | None, tz_abbreviation: str, *, short: bool = False) -> str:
    if moment is None:
        return ""
