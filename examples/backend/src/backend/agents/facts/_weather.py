...
from ._sample_widget import HourlyForecast, WeatherWidgetData
...
CURRENT_FIELDS = (
    "temperature_2m,apparent_temperature,relative_humidity_2m,is_day,wind_speed_10m,wind_direction_10m,weather_code"
)
HOURLY_FIELDS = "temperature_2m,weather_code"
DAILY_FIELDS = "weather_code,temperature_2m_max,temperature_2m_min,sunrise,sunset,precipitation_probability_max"
...
async def retrieve_weather(query: str, unit: str | None = None) -> WeatherWidgetData:
    ...
    try:
        ...
        raise WeatherLookupError("Weather data is currently unavailable for that location.") from exc
...
async def _geocode_with_nominatim(client: httpx.AsyncClient, query: str) -> GeocodedLocation:
    ...
    try:
        ...
        raise WeatherLookupError("The location data returned from the geocoder was incomplete.") from exc
...
def _build_widget_data(
    ...
    temperature_unit = current_units.get("temperature_2m") or ("°F" if unit == "fahrenheit" else "°C")
    ...
    return WeatherWidgetData(
        ...
        wind_unit=current_units.get("wind_speed_10m") or ("mph" if unit == "fahrenheit" else "km/h"),
        humidity=humidity,
...
def _weather_code_info(code: Any) -> tuple[str, str]:
    try:
        ...
        condition, icon_key = WEATHER_CODE_LOOKUP.get(numeric, ("Current conditions", DEFAULT_ICON_KEY))
