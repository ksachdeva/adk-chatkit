...
async def show_line_selector(
    ...
    # Add hidden context with available lines for the agent
    line_names = [f"{line.name} ({line.id})" for line in lines]
    await add_hidden_context(
        f"<LINE_SELECTOR_SHOWN>Available lines: {', '.join(line_names)}. Waiting for user selection.</LINE_SELECTOR_SHOWN>",
        tool_context,
    )
...
async def get_map(tool_context: ToolContext) -> dict[str, Any]:
    ...
    # Add hidden context with map summary
    station_count = len(metro_map.stations)
    line_count = len(metro_map.lines)
    await add_hidden_context(
        f"<MAP_LOADED>Map '{metro_map.name}' loaded: {station_count} stations, {line_count} lines</MAP_LOADED>",
        tool_context,
    )
...
async def list_lines(tool_context: ToolContext) -> dict[str, Any]:
    ...
    # Add hidden context with line summary
    line_info = [f"{line.name} ({line.id}, {line.color})" for line in lines]
    await add_hidden_context(
        f"<LINES_LISTED>Available lines: {', '.join(line_info)}</LINES_LISTED>",
        tool_context,
    )
...
async def list_stations(tool_context: ToolContext) -> dict[str, Any]:
    ...
    # Add hidden context with station count
    await add_hidden_context(
        f"<STATIONS_LISTED>Found {len(stations)} stations in the metro map</STATIONS_LISTED>",
        tool_context,
    )
...
async def plan_route(
    ...
    # Add hidden context with the route information
    station_names = [s.get("name", "") for s in route]
    await add_hidden_context(
        f"<ROUTE_DISPLAYED>Route planned: {' → '.join(station_names)}</ROUTE_DISPLAYED>",
        tool_context,
    )
...
async def get_station(
    ...
    # Add hidden context with station details for agent reference
    line_info = ", ".join([f"{line.name} ({line.color})" for line in valid_lines])
    await add_hidden_context(
        f"<STATION_TAG>Station: {station.name} (id: {station.id}), "
        f"coordinates: ({station.x}, {station.y}), "
        f"description: {station.description}, "
        f"lines: [{line_info}]</STATION_TAG>",
        tool_context,
    )
...
async def add_station(
    ...
    try:
        ...
        # Add hidden context so the agent knows the station was added
        position = "end" if append else "beginning"
        line = metro.find_line(line_id)
        line_name = line.name if line else line_id
        await add_hidden_context(
            f"<STATION_ADDED>Added station '{new_station.name}' (id: {new_station.id}) to the {position} of {line_name}</STATION_ADDED>",
            tool_context,
        )

        # Stream client effect to update the map in the UI
        if isinstance(run_config, ChatkitRunConfig):
            await stream_client_effect(
                name="add_station",
                data={
...
async def get_selected_stations(tool_context: ToolContext) -> dict[str, Any]:
    ...
    # Add hidden context to track that we requested station selection
    await add_hidden_context(
        "<SELECTED_STATIONS_REQUESTED>Waiting for client to return selected station IDs</SELECTED_STATIONS_REQUESTED>",
        tool_context,
    )
