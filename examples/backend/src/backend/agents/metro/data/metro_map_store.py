...
class MetroMapStore:
    ...
    def __post_init__(self) -> None:
        ...
        self._station_lookup: dict[str, Station] = {station.id: station for station in self.map.stations}
    ...
    def dump_for_client(self) -> dict[str, Any]:
        return self.map.model_dump(mode="json")

    # Mutations
    def set_map(self, map: MetroMap) -> None:
        self.map = map
    ...
    def _get_coordinates_for_new_station(self, line: Line, append: bool) -> tuple[int, int]:
        if append:
            ...
            return (prev_x + 1, prev_y) if line.orientation == "horizontal" else (prev_x, prev_y + 1)
