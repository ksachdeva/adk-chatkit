...
import type { CatColorPattern, CatStatePayload, CatSpeechPayload } from "../../lib/cat/cat";
...
export function CatStatusPanel({ onQuickAction, className, cat, speech, flashMessage }: CatStatusPanelProps) {
  const handleQuickAction = useCallback(
    (action: QuickAction) => {
      if (onQuickAction) {
        void onQuickAction(action.prompt(cat));
      }
    },
    [cat, onQuickAction]
  );
  ...
  return (
    ...
    >
      <div className="space-y-2">
        <h2 className="text-3xl font-semibold text-slate-900 dark:text-white">{cat.name}</h2>
      </div>
