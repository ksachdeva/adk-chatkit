...
import { updateMetroMap, X_UNIT, Y_UNIT, type Line } from "../../lib/metro-map/map";
...
export function MapPanel({ className }: MapPanelProps) {
  ...
  const lines: Line[] = map ? map.lines : [];
  ...
  const handleLocationSelect = useCallback(
    (x: number, y: number) => {
      // Convert from flow pixel coordinates to grid coordinates
      // Flow coordinates are already scaled (station at grid 3,3 is at flow position 480,240)
      // So we divide by the unit size and round to nearest integer
      const gridX = Math.round(x / X_UNIT);
      const gridY = Math.round(y / Y_UNIT);
      setPendingLocation({ x: gridX, y: gridY });
      setAddStationForm((prev) => ({ ...prev, x: gridX, y: gridY }));
  ...
  const handleAddStation = useCallback(async () => {
    if (!addStationForm.name.trim() || !map) {
    ...
    const newStation = {
      ...
      description: "",
      x: Math.round(addStationForm.x),
      y: Math.round(addStationForm.y),
      lines: [addStationForm.lineId],
    };

    try {
      const targetLine = map.lines.find(l => l.id === addStationForm.lineId);
      if (!targetLine) {
      ...
      const updatedMap = await updateMetroMap(
        {
          ...map,
          stations: [...map.stations, newStation],
          lines: map.lines.map(line =>
            line.id === addStationForm.lineId
